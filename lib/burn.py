...
"""Burn {} to earn {} during a special period of time.""".format(config.WDC, config.XBJ)
...
def compose (db, source, quantity, overburn=False):
    ...
    # Check that a maximum of 1,000,000 WDC total is burned per address.
    ...
    if quantity > (config.MAX_BURN_BY_ADDRESS * config.UNIT - already_burned) and not overburn:
        raise exceptions.BurnError('1,000,000 {} may be burned per address'.format(config.WDC))
...
def parse (db, tx, message=None):
    ...
    if status == 'valid':
        problems = validate(db, tx['source'], tx['destination'], tx['wdc_amount'], tx['block_index'], overburn=False)
        if problems: status = 'invalid: ' + '; '.join(problems)

        if tx['wdc_amount'] != None:
            sent = tx['wdc_amount']
        else:
            sent = 0

    if status == 'valid':
        # Calculate quantity of XBJ earned. (Maximum 1,000,000 WDC in total, ever.)
        cursor = db.cursor()
        ...
        # Credit source address with earned XBJ.
        util.credit(db, tx['block_index'], tx['source'], config.XBJ, earned, action='burn', event=tx['tx_hash'])
    else:
        burned = 0
        earned = 0
