...
def compose (db, source, quantity, overburn=False):
    ...
    # Check that a maximum of 1 LTC total is burned per address.
    ...
    if quantity > (1 * config.UNIT - already_burned) and not overburn:
        raise exceptions.BurnError('1 {} may be burned per address'.format(config.LTC))
...
def parse (db, tx, message=None):
    ...
    if status == 'valid':
        # Calculate quantity of XPT earned. (Maximum 1 LTC in total, ever.)
        cursor = db.cursor()
        ...
        util.credit(db, tx['block_index'], tx['source'], config.XPT, earned, event=tx['tx_hash'])
    else:
        burned = 0
        earned = 0
