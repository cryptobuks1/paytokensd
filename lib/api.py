...
def compose_transaction(db, name, params,
    ...
    return litecoin.transaction(tx_info, encoding=encoding,
...
def sign_transaction(unsigned_tx_hex, private_key_wif=None):
    return litecoin.sign_tx(unsigned_tx_hex, private_key_wif=private_key_wif)
...
class APIServer(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            raise Exception("Cannot start the API subsystem. Is {} already running, or is something else listening on port {}?".format(config.XPT_CLIENT, config.RPC_PORT))
