...
def insert_raw_transaction(raw_transaction, db, rawtransactions_db):
    ...
    tx = worldcoin.decode_raw_transaction(raw_transaction)
    ...
    source, destination, wdc_amount, fee, data = blocks.get_tx_info2(tx, block_index)
    transaction = (tx_index, tx_hash, block_index, block_hash, block_time, source, destination, wdc_amount, fee, data, True)
    ...
    if pytest.config.option.initrawtransactions:
        ...
        with open(CURR_DIR + '/fixtures/unspent_outputs.json', 'r') as listunspent_test_file:
                ...
                for output in wallet_unspent:
                    txid = binascii.hexlify(worldcoinlib.core.lx(output['txid'])).decode()
                    tx = worldcoin.decode_raw_transaction(output['txhex'])
...
def save_rawtransaction(db, tx_hash, tx_hex, tx_json):
    cursor = db.cursor()
    try:
        txid = binascii.hexlify(worldcoinlib.core.lx(tx_hash)).decode()
...
def run_scenario(scenario, rawtransactions_db):
    ...
    for transaction in scenario:
        if transaction[0] != 'create_next_block':
            ...
            unsigned_tx_hex = worldcoin.transaction(db, compose(db, *transaction[1]), **transaction[2])
